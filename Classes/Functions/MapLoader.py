...
class MapLoader():
    ...
    def update(self, base_node, cam, sprite_dict):

        if self.delay <= 0:
            self.generatePoints(cam, sprite_dict, base_node)
    ...
    def generatePoints(self, cam, sprite_dict, base_node):
        self.updateZoomDistance(cam, base_node)
        mpp = 512  # size of image

        scale_factor = self.getImageScaleFactor(self.zoom, getLatLongFromCoordinates(base_node, cam.origin)[0])
        mpp *= scale_factor
        key_dict = {}
        remove_dict = {}
        for x in range(int((cam.origin.getX() - cam.dim.getX()) // mpp),
                       int(((cam.origin.getX() + cam.dim.getX()) // mpp) + 1)):
            for y in range(int((cam.origin.getY() - cam.dim.getY()) // mpp),
                           int(((cam.origin.getY() + cam.dim.getY()) // mpp) + 1)):
                key = self.constructKey(x, y, self.zoom)
                directory = self.cwd + '/img/Map/' + key + '.png'
                if key not in sprite_dict:
                    point = getLatLongFromCoordinates(base_node, Vector(x * mpp, y * mpp))
                    ...
                    elif key not in self.loading_keys:
                        imgThread = threading.Thread(target=self.loadImage, args=(url, directory))
                        imgThread.start()
                        self.loading_keys.update({key: key})

                    # if image in file load from file else load from simpleguics2 as bellow
                    # else load image from url and save to directory

                    if image != 0:
                        tile = SpriteAnimator(image, scale_factor)
                        radius = 256 * scale_factor
                        sprite_dict.update({key: tile})

                        map_tile = MapTile(Vector(x * mpp, y * mpp), self.zoom, key, sprite_dict, radius)
                        self.tile_dict.update({key: map_tile})
    ...
    def updateZoomDistance(self, cam, base_node):
        zoom = 20
        for i in range(0, 19):
            j = 20 - i
            if getMetersPerPixelGoogleImage(getLatLongFromCoordinates(base_node, cam.origin)[0],
                                            j) > getMetersPerPixelsCam(cam):
                break
            else:
                zoom = j
        self.zoom = zoom - 1
